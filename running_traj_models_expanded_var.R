...
for (longi_var_idx in 4:length(names(long_data_ts))){
...
for (fold in 1:nfolds){
  ...
  model_name <- 'cox_expanded_var_traj_plus_data_y15_gap_3'
  gc()
...
for (fold in 1:nfolds){
  ...
  model_name <- 'lasso_expanded_var_traj_plus_data_y15_gap_3'
  gc()
  main_dir <- paste0(work_dir, '/rdata_files')
  sub_dir <- paste0(model_name, '_fold_',fold)
