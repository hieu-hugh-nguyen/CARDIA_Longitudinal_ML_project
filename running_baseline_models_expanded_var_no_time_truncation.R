...
# baseline data:
data_at_baseline <- data_longi_long_for_analysis %>% dplyr::filter(ID %in% subjects_in_cohort[[1]]) %>% 
                                                              dplyr::filter(!duplicated(ID, fromLast=FALSE)) 
...
# No truncation time (predict risk after Y0 to Y30):
