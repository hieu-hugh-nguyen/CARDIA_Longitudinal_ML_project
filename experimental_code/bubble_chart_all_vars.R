...
longi_data_dict <- read.csv(paste0(work_dir,'/csv_files/longi_data_availability_dictionary_filtered.csv'))
...
bubble_longi_data_dict_dup_rm <- longi_data_dict_dup_rm %>% arrange(exam_year) %>% dplyr::select(exam_year, varname_longi, non_missing_per) %>% 
  ...
  left_join(longi_data_dict_dup_rm %>% select(var_group, varname_longi,Variable.Label) %>% dplyr::filter(duplicated(varname_longi) == FALSE),by = 'varname_longi') %>%
  dplyr::select(var_group, varname_longi,Variable.Label,everything())

write.csv(bubble_longi_data_dict_dup_rm %>% apply(2,as.character), file = paste0(work_dir,'/csv_files/longi_data_avalability_dup_rm_bubble_format3.csv'), row.names = F)
