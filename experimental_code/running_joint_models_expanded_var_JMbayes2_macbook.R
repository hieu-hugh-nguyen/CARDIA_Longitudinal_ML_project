...
# fold 6 has glu  convergence problem
for (fold in 7:nfolds){
  ...
  data_cox = data_at_baseline %>% dplyr::filter(ID %in% train_id) %>%
    dplyr::select(-time) %>% dplyr::filter(time_te_in_yrs >0) %>%
    dplyr::rename(event = status) %>% dplyr::rename(time = time_te_in_yrs)
...
#                        x=TRUE))
  ...
  library(dplyr)
  model_name <- 'jmbayes2_expanded_var_macbook'
  gc()
 ...
 # tryCatch({
  ...
  #                                    ,random =~ exam_year | ID
  #                                    ,family = binomial())
  # not converged! Warning: Unstable solution
  ...
  CoxFit <- coxph(Surv(time, event) ~ AGE_Y0+MALE+RACEBLACK+ARMCI+ASMA+BEER+CANCR+
                  CGTDY+CHOL+DFPAY+DIAB+ED+GALL+KIDNY+LIFE+LIQR+LIVER+MENTL+NPREG+
  ...
  #   roc <- tvROC(model, newdata = test_data, Tstart = 15, Thoriz = eval_time)
  #   auc <- tvAUC(roc)
  ...
  # predLong <- predict(jointFit, newdata =  test_data
  #                     , process = "longitudinal"
  #                     ,times = eval_times,
  #                     ,type = "subject_specific"
  #                     ,return_newdata = TRUE
  # )
  predSurv <- predict(model, newdata = test_data %>% 
                        dplyr::select(-one_of(c('event', 'time')))
                      , process = "event"
                      ,times = eval_times,
                      ,type = "subject_specific"
                      ,return_newdata = TRUE
  ...
  # plot(predLong, predSurv,  subject ='100056526386')
  
  prob_risk_test_with_ID <- predSurv %>% dplyr::select(c('ID', 'exam_year', 'pred_CIF')) %>%
    stats::reshape(idvar = 'ID', timevar= 'exam_year', direction = 'wide')
  ...
  pred_names <- NULL
  for (eval_time in eval_times){
    pred_names <- c(pred_names,paste0('pred_CIF','.',eval_time))
  }
  prob_risk_test <- prob_risk_test_with_ID %>% dplyr::select(pred_names) %>% as.matrix()
  prob_risk_test[is.na(prob_risk_test)] = 0
  row.names(prob_risk_test) <- NULL
  
  performance_testset = eval_performance3_5(prob.risk.test.set = prob_risk_test
                                          , test.data = test_data %>% filter(!duplicated(ID, fromLast=FALSE))
                                          , trained.data = trained_data %>% filter(!duplicated(ID, fromLast=FALSE)) 
...
# Error in lme.formula(GLU ~ exam_year * MALE * RACEBLACK + AGE_Y0, data = train_data,  : 
