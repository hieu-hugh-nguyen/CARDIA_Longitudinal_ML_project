...
def create_rnn_cell(num_units, num_layers, keep_prob, RNN_type, activation_fn): 
    ...
    for _ in range(num_layers):
        if RNN_type == 'GRU':
            cell = tf.compat.v1.nn.rnn_cell.GRUCell(num_units, activation=activation_fn)
        elif RNN_type == 'LSTM':
            cell = tf.compat.v1.nn.rnn_cell.LSTMCell(num_units, activation=activation_fn, state_is_tuple=True)
            # cell = tf.contrib.rnn.LSTMCell(num_units, activation=activation_fn)
        if not keep_prob is None:
            cell = tf.compat.v1.nn.rnn_cell.DropoutWrapper(cell, input_keep_prob=keep_prob, output_keep_prob=keep_prob) # state_keep_prob=keep_prob
        cells.append(cell)
...
def create_FCNet(inputs, num_layers, h_dim, h_fn, o_dim, o_fn, w_init, w_reg=None, keep_prob=1.0):
    ...
    if w_init is None:
        w_init = tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform") # Xavier initialization
