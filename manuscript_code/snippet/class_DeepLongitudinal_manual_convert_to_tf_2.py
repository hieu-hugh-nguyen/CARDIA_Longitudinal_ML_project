...
#        with tf.variable_scope(self.name):
        with tf.compat.v1.variable_scope(self.name):
            ...
            x_last = tf.compat.v1.reduce_sum(tf.tile(tf.expand_dims(self.rnn_mask2, axis=2), [1,1,self.x_dim]) * self.x, reduction_indices=1)    #sum over time since all others time stamps are 0
            ...
            mi_last = tf.compat.v1.reduce_sum(tf.tile(tf.expand_dims(self.rnn_mask2, axis=2), [1,1,self.x_dim]) * self.x_mi, reduction_indices=1)    #sum over time since all others time stamps are 0
            ...
            rnn_mask_att   = tf.cast(tf.not_equal(tf.compat.v1.reduce_sum(x_hist, reduction_indices=2), 0), dtype=tf.float32)  #[mb_size, max_length-1], 1:measurements 0:no measurements
            ...
            self.att_weight  = div(att_weight,(tf.compat.v1.reduce_sum(att_weight, axis=1, keepdims=True) + _EPSILON)) #softmax (tf.exp is done, previously)

            # 1) expand att_weight to hidden state dimension, 2) c = \sum_{j=1}^{M} a_{j} x h_{j}
            self.context_vec = tf.compat.v1.reduce_sum(tf.tile(tf.reshape(self.att_weight, [-1, self.max_length-1, 1]), [1, 1, self.num_layers_RNN*self.h_dim1]) * rnn_states, axis=1)
...
#             self.z_mean      = FC_Net(rnn_outputs, self.x_dim, activation_fn=None, weights_initializer=self.initial_W, scope="RNN_out_mean1")
            ...
            h = tf.compat.v1.nn.dropout(h, keep_prob=self.keep_prob)
    ...
    def loss_Log_Likelihood(self):
        ...
        I_1 = tf.sign(self.k)
        denom = 1 - tf.compat.v1.reduce_sum(tf.compat.v1.reduce_sum(self.fc_mask1 * self.out, reduction_indices=2), reduction_indices=1, keepdims=True) # make subject specific denom.
        ...
        tmp1 = tf.compat.v1.reduce_sum(tf.compat.v1.reduce_sum(self.fc_mask2 * self.out, reduction_indices=2), reduction_indices=1, keepdims=True)
        tmp1 = I_1 * log(div(tmp1,denom))

        #for censored: log \sum P(T>t|x,Y,t>t_M)
        tmp2 = tf.compat.v1.reduce_sum(tf.compat.v1.reduce_sum(self.fc_mask2 * self.out, reduction_indices=2), reduction_indices=1, keepdims=True)
    ...
    def loss_Ranking(self):
        ...
        self.LOSS_2 = tf.compat.v1.reduce_sum(eta) #sum over num_Events
    ...
    def loss_RNN_Prediction(self):
        ...
        zeta = tf.reduce_mean(tf.compat.v1.reduce_sum(tmp_mask1 * (1. - tmp_mi) * tf.pow(self.z - tmp_x, 2), reduction_indices=1))  #loss calculated for selected features.
