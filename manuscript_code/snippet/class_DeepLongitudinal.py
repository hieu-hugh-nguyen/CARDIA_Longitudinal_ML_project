...
def get_seq_length(sequence):
    ...
    tmp_length = tf.reduce_sum(input_tensor=used, axis=1)
...
class Model_Longitudinal_Attention:
    ...
    def __init__(self, sess, name, input_dims, network_settings):
        ...
        self.reg_W              = tf.keras.regularizers.l1(l=network_settings['reg_W'])
        self.reg_W_out          = tf.keras.regularizers.l1(l=network_settings['reg_W_out'])
    ...
    def _build_net(self):
        with tf.compat.v1.variable_scope(self.name):
            ...
            self.keep_prob   = tf.compat.v1.placeholder(tf.float32)                                                      #keeping rate
            self.a           = tf.compat.v1.placeholder(tf.float32)
            self.b           = tf.compat.v1.placeholder(tf.float32)
            self.c           = tf.compat.v1.placeholder(tf.float32)

            self.x           = tf.compat.v1.placeholder(tf.float32, shape=[None, self.max_length, self.x_dim])
            self.x_mi        = tf.compat.v1.placeholder(tf.float32, shape=[None, self.max_length, self.x_dim])           #this is the missing indicator (including for cont. & binary) (includes delta)
            self.k           = tf.compat.v1.placeholder(tf.float32, shape=[None, 1])                                     #event/censoring label (censoring:0)
            self.t           = tf.compat.v1.placeholder(tf.float32, shape=[None, 1])


            self.fc_mask1    = tf.compat.v1.placeholder(tf.float32, shape=[None, self.num_Event, self.num_Category])     #for denominator
            self.fc_mask2    = tf.compat.v1.placeholder(tf.float32, shape=[None, self.num_Event, self.num_Category])     #for Loss 1
            self.fc_mask3    = tf.compat.v1.placeholder(tf.float32, shape=[None, self.num_Category])                     #for Loss 2
            ...
            def loop_fn_att(time, cell_output, cell_state, loop_state):
                ...
                finished = tf.reduce_all(input_tensor=elements_finished)    
                next_input = tf.cond(pred=finished, true_fn=lambda: tf.zeros([self.mb_size, 2*self.x_dim], dtype=tf.float32),  # [x_hist, mi_hist]
            ...
            x_last = tf.reduce_sum(input_tensor=tf.tile(tf.expand_dims(self.rnn_mask2, axis=2), [1,1,self.x_dim]) * self.x, axis=1)    #sum over time since all others time stamps are 0
            ...
            mi_last = tf.reduce_sum(input_tensor=tf.tile(tf.expand_dims(self.rnn_mask2, axis=2), [1,1,self.x_dim]) * self.x_mi, axis=1)    #sum over time since all others time stamps are 0
            ...
            rnn_mask_att   = tf.cast(tf.not_equal(tf.reduce_sum(input_tensor=x_hist, axis=2), 0), dtype=tf.float32)  #[mb_size, max_length-1], 1:measurements 0:no measurements
            ...
            rnn_outputs_ta, self.rnn_final_state, loop_state_ta = tf.compat.v1.nn.raw_rnn(cell, loop_fn_att)
            ...
            self.att_weight  = div(att_weight,(tf.reduce_sum(input_tensor=att_weight, axis=1, keepdims=True) + _EPSILON)) #softmax (tf.exp is done, previously)

            # 1) expand att_weight to hidden state dimension, 2) c = \sum_{j=1}^{M} a_{j} x h_{j}
            self.context_vec = tf.reduce_sum(input_tensor=tf.tile(tf.reshape(self.att_weight, [-1, self.max_length-1, 1]), [1, 1, self.num_layers_RNN*self.h_dim1]) * rnn_states, axis=1)


            self.z_mean      = _layers.fully_connected(rnn_outputs, self.x_dim, activation_fn=None, weights_initializer=self.initial_W, scope="RNN_out_mean1")
            self.z_std       = tf.exp(_layers.fully_connected(rnn_outputs, self.x_dim, activation_fn=None, weights_initializer=self.initial_W, scope="RNN_out_std1"))

            epsilon          = tf.random.normal([self.mb_size, self.max_length-1, self.x_dim], mean=0.0, stddev=1.0, dtype=tf.float32)
            ...
            h = _layers.fully_connected(inputs, self.h_dim2, activation_fn=self.FC_active_fn, weights_initializer=self.initial_W, scope="Layer1")
            ...
            self.LOSS_BURNIN    = self.LOSS_3 + tf.compat.v1.losses.get_regularization_loss()

            self.solver         = tf.compat.v1.train.AdamOptimizer(learning_rate=self.lr_rate).minimize(self.LOSS_TOTAL)
            self.solver_burn_in = tf.compat.v1.train.AdamOptimizer(learning_rate=self.lr_rate).minimize(self.LOSS_BURNIN)
    ...
    def loss_Log_Likelihood(self):
        ...
        I_1 = tf.sign(self.k)
        denom = 1 - tf.reduce_sum(input_tensor=tf.reduce_sum(input_tensor=self.fc_mask1 * self.out, axis=2), axis=1, keepdims=True) # make subject specific denom.
        ...
        tmp1 = tf.reduce_sum(input_tensor=tf.reduce_sum(input_tensor=self.fc_mask2 * self.out, axis=2), axis=1, keepdims=True)
        tmp1 = I_1 * log(div(tmp1,denom))

        #for censored: log \sum P(T>t|x,Y,t>t_M)
        tmp2 = tf.reduce_sum(input_tensor=tf.reduce_sum(input_tensor=self.fc_mask2 * self.out, axis=2), axis=1, keepdims=True)
        tmp2 = (1. - I_1) * log(div(tmp2,denom))

        self.LOSS_1 = - tf.reduce_mean(input_tensor=tmp1 + sigma3*tmp2)
    ...
    def loss_RNN_Prediction(self):
        ...
        zeta = tf.reduce_mean(input_tensor=tf.reduce_sum(input_tensor=tmp_mask1 * (1. - tmp_mi) * tf.pow(self.z - tmp_x, 2), axis=1))  #loss calculated for selected features.
