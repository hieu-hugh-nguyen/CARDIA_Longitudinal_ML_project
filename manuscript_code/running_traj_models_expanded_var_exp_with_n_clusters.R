...
criteria_method_list = c(
                    # "ch"
                    # , "kl", "ccc", "hartigan", "scott"
...
cluster_all_df_list <- list()


for (criteria_method in criteria_method_list){
  ...
  # cluster_all_df_fixed_var <- longi_data %>% dplyr::select(one_of(c('ID','event','time',fixed_var))) %>%
  #   dplyr::filter(!duplicated(ID, fromLast=TRUE))
  # cluster_all_df_fixed_var$ID <- cluster_all_df_fixed_var$ID %>% as.character()
  # 
  # for (criteria_method in criteria_method_list){
  #   cluster_all_df_list[[criteria_method]] = cluster_all_df_fixed_var
  # }
  ...
  for (longi_var_idx in 3:length(names(long_data_ts))){
    ...
    if(!is.null(s1)){
     ...
     # for (criteria_method in criteria_method_list){
        ...
        s3 <- step3clusters(s2, criteria= criteria_method)
        ...
        #                   , index= criteria_method, method = 'kmeans', , min.nc=2, max.nc=6)
    ...
    #    cluster_all_df_list[[criteria_method]] <- cluster_all_df_list[[criteria_method]] %>% inner_join(cluster_assign_df %>% dplyr::select(c('ID', paste0(longi_var, '_cluster'))), by = 'ID')
      #}
    ...
    #   s3_with_NA=step3clusters(s2_with_NA, criteria= criteria_method)
  ...
  write.csv(cluster_all_df, file = paste0(work_dir, '/csv_files/proc_traj_cluster_assignments_',  criteria_method, '_4.csv'), row.names = FALSE)
  
  save(s3_all, file = paste0(work_dir, '/rdata_files/proc_traj_s3_object_',  criteria_method, '_4.RData'))
  save(s3_all_with_NA, file = paste0(work_dir, '/rdata_files/proc_traj_s3_object_with_NA_',  criteria_method, '_4.RData'))
  ...
  # criteria_method = 'trcovw'
  # s3_load <- get(load(paste0(work_dir, '/rdata_files/proc_traj_s3_object_',  criteria_method, '_4.RData')))
  # plotCombTraj(s3_load$SBP_s3)
  # s3_with_NA_load <- get(load(paste0(work_dir, '/rdata_files/proc_traj_s3_object_with_NA_',  criteria_method, '_4.RData')))
  ...
  cluster_all_df <- read.csv(paste0(work_dir, '/csv_files/proc_traj_cluster_assignments_',  criteria_method, '_4.csv'))
  ...
  # data <- data_and_cluster_df %>% dplyr::select(-one_of(c("ARMCI", "BMI","CHOL","DFPAY"
  ...
  for (fold in 1:nfolds){
    ...
    model_name <- paste0('cox_expanded_var_traj_only_',  criteria_method, '_4')
 #   model_name <- paste0('cox_expanded_var_traj_',  criteria_method, '_plus_some_y15_data_4')
    gc()
  ...
  for (fold in 1:nfolds){
    ...
    model_name <- paste0('lasso_expanded_var_traj_only_',  criteria_method, '_4')
    
#    model_name <- paste0('lasso_expanded_var_traj_',  criteria_method, '_plus_some_y15_data_4')
    gc()
    main_dir <- paste0(work_dir, '/rdata_files')
    sub_dir <- paste0(model_name, '_fold_',fold)
  ...
  for (fold in 1:nfolds){
    ...
    model_name <- paste0('rsf_expanded_var_traj_only_',  criteria_method, '_4')
    
    # model_name <- paste0('rsf_expanded_var_traj_',  criteria_method, '_plus_some_y15_data_6')
    gc()
    main_dir <- paste0(work_dir, '/rdata_files')
    sub_dir <- paste0(model_name, '_fold_',fold)
