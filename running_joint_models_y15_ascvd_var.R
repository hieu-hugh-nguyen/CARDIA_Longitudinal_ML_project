...
for (fold in c(9,6,5,4)){
 # fold = 
  ...
  # trained_model <- model
  trained_model <- get(load(paste0(saving_dir,'/', model_name, '.RData')))
...
#    prob_risk_test_with_ID <- prob_risk_test
    ...
    #     prob_risk_train <- predictRisk.rsf(trained.model = model
