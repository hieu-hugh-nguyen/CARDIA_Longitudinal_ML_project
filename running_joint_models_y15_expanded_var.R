...
# baseline data:
data_at_baseline <- data_longi_analysis_cohort %>% dplyr::filter(!duplicated(ID, fromLast=FALSE)) 
...
for (fold in 1:nfolds){
 # fold = 1
  ...
  model_name <- 'jmbayes_expanded_var_y15'
  gc()
  ...
  MixedModelFit <-JMbayes:: mvglmer(list(CHOL ~ exam_year + MALE + RACEBLACK + (exam_year | ID)
                                      ...
                                      #   , DBP ~ exam_year + MALE + RACEBLACK + (exam_year | ID)
                                         , BMI ~ exam_year + MALE + RACEBLACK + (exam_year | ID)
                                     #    , LDL ~ exam_year + MALE + RACEBLACK + (exam_year | ID)
                                     #    , NTRIG ~ exam_year + MALE + RACEBLACK + (exam_year | ID)
                                      #   , PSTYR ~ exam_year + MALE + RACEBLACK + (exam_year | ID)
                                      #   , PULSE ~ exam_year + MALE + RACEBLACK + (exam_year | ID)
                                      #   , WST ~ exam_year + MALE + RACEBLACK + (exam_year | ID)
