...
plot_traj <- function(var_oi = 'SBP'){
  ...
  if (paste0(var_oi, '_cluster') %in% names(cluster_all_df)){
...
plot_traj_mean <- function(var_oi = 'SBP'){
...
ranked_var_list_df <- read.csv(paste0(work_dir,'/csv_files/median_outerloop_permut_vimp_rsf_expanded_var_traj_cont_n_binary_var_only_trcovw_6.csv'))

#ranked_var_list <- names(longi_data)[8:length(names(longi_data))] 
...
ggsave(filename = 'median_traj.tif', path = paste0(work_dir, '/figures')
...
plot_all_obj_mean <- plot_traj_mean(ranked_var_list[1])
for (var_idx in 2:length(ranked_var_list)){
  plot_all_obj_mean <- plot_all_obj_mean +
    plot_traj_mean(ranked_var_list[var_idx]) 
  
}

plot_all_obj_mean

ggsave(filename = 'mean_traj.tif', path = paste0(work_dir, '/figures')
       , units = 'in', width = 7.5, height = 7
       , device = 'tiff', dpi = 300)